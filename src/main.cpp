...
int mArray, nArray;
string fin = "", fout;   //input-output file names
...
int main(int argc, char **argv){
    ...
    void datFileInput(string fin);
    void sortColumnsR();
    void sortColumns();
    void outputConceptsByLevels();
    void sortRows();
    void read_param(int argc,char*argv [], std::string& fin, bool& strat_int, int& max_level_id, bool& output_itemsets);
    
    string strategy_name = "I"; // GDPM-INT
    ...
    struct stat buffer; int check = 0;
    ...
    if (stat (fout.c_str(), &buffer) != 0){
        ...
        cout << check << "  " << stat (fout.c_str(), &buffer) << "  " << fout << endl;
    }
    if (check == 0){
        fout += "/dat/";
        check = mkdir(fout.c_str(), 0777);
    ...
    if (check != 0){
        cout << "Unable to create an output directory" <<endl;
    }

    
    cout << "Intput file " << fin << endl;
    cout << "Output file " << fout << endl;
    
    datFileInput(fin); // reading dataset
    cout << "Dataset of size " << n << " x " << m << endl;
...
//    sortRows();
    ...
    if (write_zero)
        write_itemsets(fout + to_string(0), A_list, B_list, 1, n, m, colOriginal, attributes);
    ...
    int last_cid = 1;
    int total_sum = 0;
    while (run and level_id < max_level_id){
        int new_last_cid = 0;
        if (strat_int)
        ...
        cout << "\n OUTPUT " << level_id << " " << new_last_cid << "; time  " << time_splits[level_id]<< endl;
        cout << " size : " << n_nodes(root) << endl;
        run = new_last_cid > 0;
        if (output_itemsets and run)
                    write_itemsets(fout + to_string(level_id), A_list, B_list, new_last_cid, n, m, colOriginal, attributes);
        //        read_itemsets("res" + to_string(level_id) + ".csv");
        last_cid = new_last_cid;
        level_size[level_id] = new_last_cid;
        tree_size_array[level_id] = n_nodes(root);
    }
    write_summary(fout + "summary_" + strategy_name + ".csv", time_splits, level_size, tree_size_array, level_id);
}
    ...
    for(int cid = 0; cid < last_cid; cid++){
        ...
        for (int i = Iter_list[cid]; i < m; i++){
            if (!(Bc[i>>6] & (VECTOR_1 << (i % 64)))){ // compute next
                ...
                int size = 0;
                for(int j = 0; j < n; j++)
                    if((Ac[j>>6] & (VECTOR_1 << (j % 64)))and(context[j][i>>6] & (VECTOR_1 << (i % 64)))){
                ...
                if (size > 0){
                    ...
                    if (c_node != nullptr){
                        ...
                        for (int ii = 0; ii < m; ii++){
                            if (!((Bc[ii>>6] & (VECTOR_1 << (ii % 64))) and (ii != i))){
        ...
        for (int i = Iter_list[cid]; i < m; i++){
            if (!(Bc[i>>6] & (VECTOR_1 << (i % 64)))){ // compute next
                ...
                if (size > 0){
                    ...
                    memcpy(Bc_new,Bc,mArray*8);
                    for (int ii = 0; ii < m; ii++){
                        if (!((Bc[ii>>6] & (VECTOR_1 << (ii % 64))) and (ii != i))){// all exept of closed and i
                            bool add = true; int jj = 0;
                            while ((jj < n) and add){
                                if (Ac_new[jj>>6] & (VECTOR_1<< (jj % 64)))
                                    if(!(context[jj][ii>>6] & (VECTOR_1 << (ii % 64))))
...
void datFileInput(string fin) {
    ...
    n=count(istreambuf_iterator<char>(datFile), istreambuf_iterator<char>(), '\n');
    nArray = (n-1)/64 + 1;
    ...
    while(getline(datFile,line)) {
        ...
        while(getline(s, cell, ' ')) {
            ...
            if(k != attributes.end()) {
                int j;
                istringstream(cell) >> j;
                contextTemp[attribute_indices[j]][(r>>6)] |= (VECTOR_1 <<(r%64));    //set context bit to true where I is byte: i div 8, bit: i mod 8
    ...
    if (c < argc){
        ...
        while (c < argc){
            switch (argv[c][0]){
                ...
                case '-':  if (argv[c][1]=='n'){max_level_id = std::stoi(argv[c + 1]); c++;} break;
            }
            c++;
        }
    }
